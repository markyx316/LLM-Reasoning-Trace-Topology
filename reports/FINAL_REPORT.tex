...
text-saturation, this argues for a deliberately
...
\emph{Grace} HPC cluster, with jobs scheduled via SLURM on NVIDIA B200 and H200 GPUs from the \texttt{gpu_b200} and \texttt{gpu_h200} partition. Software
...
weight decay $0.01$, peak learning rate $2 \times 10^{-5}$, linear warmup
over $10\%$ of steps then linear decay, batch size $16$, $5$ epochs. Models
are loaded in fp32 (an early bf16 fallback caused saturation at AUROC $0.5$
and was patched). Training is repeated under $5$-fold stratified CV with
\texttt{random\_state=42}; out-of-fold probabilities are saved as the OOF
input to the meta-learner.

\paragraph{Multi-layer hidden-state probe} (signal $P_{\text{probe}}$).
Hidden states are extracted at the answer-marker token at four decoder
depths (layers $\{8, 16, 24, 32\}$ for the $32$-layer Qwen-7B and Llama-8B
distillations) by a single teacher-forcing pass over the saved trace.
Probe head: 2-layer MLP with hidden size $256$, ReLU activations, dropout
$0.3$, sigmoid output. Optimizer AdamW, learning rate $10^{-3}$, weight
decay $10^{-4}$, batch size $128$, up to $30$ epochs with early stopping
on validation AUROC (patience $5$). $5$-fold stratified CV;
...
\item \emph{Logistic regression.} L1 penalty, robust scaler on inputs.
Inverse-regularization strength $C$ selected by inner $5$-fold CV over
$\{10^{-3}, 10^{-2}, \ldots, 10\}$.
\item \emph{Random forest.} $200$ trees, $\text{max\_depth}=\text{None}$,
$\text{min\_samples\_leaf}=5$, $\text{class\_weight}=\text{balanced}$,
\texttt{random\_state=42}.
\item \emph{XGBoost.} Selected by an Optuna~\cite{akiba2019optuna} study
of $2{,}000$ trials maximising mean $5$-fold CV AUROC. Search space:
...
$\lambda \in [10^{-3}, 10]$, $\alpha \in [10^{-3}, 10]$. Final config:
$\eta=0.05$, max\_depth $6$, subsample $0.8$, colsample\_bytree $0.7$,
n\_estimators $400$, $\gamma=0.1$, $\lambda=1.0$, $\alpha=10^{-3}$.
\end{itemize}
