...
\author{
  ...
  Peng Chen \\
  \texttt{peng.chen.pc838@yale.edu} \\
  Lixing Lin \\
...
\newblock \emph{arXiv:2506.05744}, 2025.
...
\newblock \emph{arXiv:2302.09664}, 2023.
...
The code for SuperHybrid is available at \url{https://github.com/markyx316/LLM-Reasoning-Trace-Topology}
...
\emph{Grace} HPC cluster, with jobs scheduled via SLURM on NVIDIA B200 and H200 GPUs from the \texttt{gpu\_b200} and \texttt{gpu\_h200} partition. Software
...
following this NeurIPS format, and make sure it's compliant with all the requirements.''

\item \emph{Model selection.} ``We keep getting the same DeBERTa
results. We need a genuinely new method. Search for alternatives and
recommend the best option for improving AUROC on long reasoning traces.''

\item \emph{Performance diagnosis.} ``Check why training is so slow---each
epoch takes 24 minutes. Identify the bottleneck and fix it.''

\item \emph{Feature engineering.} ``Implement focal loss, label
smoothing, and early stopping in the Longformer fine-tuning loop. Add
\texttt{--focal-gamma}, \texttt{--label-smoothing}, and
\texttt{--patience} arguments so we can sweep them from the SLURM
script.''

\item \emph{Infrastructure.} ``The step-embedding builder gets killed on
the login node. Write an \texttt{sbatch} script that runs it on a GPU
node with appropriate memory and time limits.''

\item \emph{Result interpretation.} ``The ensemble only gains $+0.001$
AUROC over the Longformer alone. What does this tell us about the
marginal value of step embeddings, and what should we try next?''

\item \emph{Compatibility.} ``We got \texttt{CUDA error: no kernel image
is available for execution on the device} on the B200 partition.
Diagnose and fix the PyTorch--CUDA architecture mismatch.''

\end{enumerate}

\includepdf[pages=-]{Reproducibility_checklist.pdf}
